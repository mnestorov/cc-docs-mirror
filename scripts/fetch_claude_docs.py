...
def validate_markdown_content(content: str, filename: str) -> None:
    """
    Validate that content is proper markdown with improved checks.
    ...
    html_indicators = ['<!DOCTYPE', '<html', '<head>', '<body>', '<?xml']
    content_start = content[:200].lower()
    if any(indicator.lower() in content_start for indicator in html_indicators):
        raise ValueError("Received HTML/XML instead of markdown")
    
    # Check for error pages
    error_indicators = ['404 not found', '403 forbidden', '500 internal server error', 'access denied']
    if any(error in content.lower()[:500] for error in error_indicators):
        raise ValueError("Received error page instead of content")
    
    # Check minimum length (increased threshold)
    if len(content.strip()) < 100:
        raise ValueError(f"Content too short ({len(content)} bytes, minimum 100)")
    ...
    # Count markdown indicators (improved logic)
    indicator_count = 0
    unique_indicators = set()
    for line in lines[:100]:  # Check first 100 lines
        line_stripped = line.strip()
        for indicator in markdown_indicators:
            if line_stripped.startswith(indicator) or (indicator in line and len(line_stripped) > 2):
                if indicator not in unique_indicators:
                    unique_indicators.add(indicator)
                    indicator_count += 1
                if indicator_count >= 5:  # Early exit if we have enough indicators
                    break
        if indicator_count >= 5:
            break
    
    # Require at least some markdown formatting (relaxed threshold)
    if indicator_count < 2:
        logger.warning(f"Content for {filename} has minimal markdown formatting ({indicator_count} indicators)")
    
    # Check for common documentation patterns (expanded)
    doc_patterns = [
        'installation', 'setup', 'usage', 'example', 'api', 'configuration', 
        'claude', 'code', 'documentation', 'guide', 'tutorial', 'reference',
        'getting started', 'quickstart', 'overview', 'introduction'
    ]
    ...
    suspicious_patterns = [
        'login required', 'please sign in', 'authentication required',
        'rate limit exceeded', 'too many requests'
    ]
    if any(pattern in content_lower for pattern in suspicious_patterns):
        raise ValueError(f"Content appears to be blocked or rate-limited")


def fetch_markdown_content(path: str, session: requests.Session, base_url: str, show_progress: bool = False) -> Tuple[str, str]:
    """
    ...
    if not show_progress:  # Only log if not using progress bars
        logger.info(f"Fetching: {markdown_url} -> {filename}")
    
    for attempt in range(MAX_RETRIES):
        try:
            response = session.get(markdown_url, headers=HEADERS, timeout=REQUEST_TIMEOUT, allow_redirects=True)
...
def main():
    """Main function with improved robustness and progress visualization."""
    start_time = datetime.now()
    
    # Startup banner
    print("🚀 Claude Code Documentation Fetcher v1.0")
    print("=" * 50)
    if TQDM_AVAILABLE:
        print("📊 Progress visualization: ✅ Enabled")
    else:
        print("📊 Progress visualization: ❌ Disabled (install tqdm for progress bars)")
    ...
    with requests.Session() as session:
        ...
        # Fetch each discovered page with progress bar
        if TQDM_AVAILABLE:
            progress_bar = tqdm(
                documentation_pages, 
                desc="📥 Fetching docs", 
            ...
            progress_bar = documentation_pages
            logger.info(f"Fetching {len(documentation_pages)} documentation pages...")
        
        for i, page_path in enumerate(progress_bar, 1):
            if not TQDM_AVAILABLE:
                logger.info(f"Processing {i}/{len(documentation_pages)}: {page_path}")
            else:
                # Update progress bar description with current page
                page_name = page_path.split('/')[-1] if '/' in page_path else page_path
                progress_bar.set_description(f"📥 Fetching {page_name}")
            
            try:
                filename, content = fetch_markdown_content(page_path, session, base_url, show_progress=TQDM_AVAILABLE)
    ...
    if TQDM_AVAILABLE:
        changelog_progress = tqdm(
            [1], 
            desc="📋 Fetching changelog", 
            unit="file",
            bar_format="{l_bar}{bar:30}{r_bar}",
            colour="blue"
        )
        for _ in changelog_progress:
            pass  # Just to show the progress bar
    else:
    ...
    # Enhanced summary with progress visualization
    duration = datetime.now() - start_time
    success_rate = (successful / len(documentation_pages)) * 100 if documentation_pages else 0
    ...
    print(f"📄 Total pages discovered: {len(documentation_pages)}")
    print(f"✅ Successfully fetched: {successful}")
    print(f"❌ Failed: {failed}")
    print(f"📈 Success rate: {success_rate:.1f}%")
    print(f"💾 Total files saved: {len(fetched_files)}")
    
    if TQDM_AVAILABLE and successful > 0:
    ...
    if failed_pages:
        print(f"\n⚠️  Failed pages (will retry next run):")
