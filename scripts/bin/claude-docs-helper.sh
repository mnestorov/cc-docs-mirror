...
# Claude Code Documentation Helper Script v1.0.0
...
auto_update() {
    ...
    if ! git fetch --quiet origin "$BRANCH" 2>/dev/null; then
        ...
        if ! git fetch --quiet origin main 2>/dev/null; then
            ...
            # Check if we're offline by trying to ping GitHub
            if ! ping -c 1 -W 2 github.com >/dev/null 2>&1; then
                return 2  # Offline
            fi
            return 2  # Other network issue
        fi
    ...
    if [[ "$LOCAL" != "$REMOTE" ]] && [[ "$BEHIND" -gt 0 ]]; then
        ...
        if [[ $VERSION_INT -ge 3 ]]; then
            ...
            ./scripts/bin/install.sh >/dev/null 2>&1
        fi
    fi
...
uninstall() {
    ...
    echo ""
    echo "  ~/.claude-code-docs/scripts/bin/uninstall.sh"
    echo ""
    echo "Or to skip confirmation:"
    echo "  echo 'y' | ~/.claude-code-docs/scripts/bin/uninstall.sh"
    echo ""
}
